...
async fn stress_boundary_timestamps() {
    ...
    let id = wal
        .append(WalOperation::CreateAgent, &actor, Bytes::from("t0"))
        .await
        .unwrap();
    ...
    let id2 = wal
        .append(WalOperation::UpdateAgent, &actor, Bytes::from("big"))
        .await
        .unwrap();
...
async fn stress_cleanup_at_exact_boundary() {
    ...
    // Create entry at t=0
    let id = wal
        .append(WalOperation::CreateAgent, &actor, Bytes::from("data"))
        .await
        .unwrap();
    ...
    assert_eq!(
        removed, 0,
        "entry at exact boundary should NOT be removed (< not <=)"
    );
    ...
    assert_eq!(
        removed2, 1,
        "entry should be removed when threshold > completion time"
    );
}
