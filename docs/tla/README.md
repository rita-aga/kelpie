...
### KelpieLease - Safe Configuration (should pass)
```bash
cd docs/tla
...
3171 states generated, 679 distinct states found, 0 states left on queue.
```

### KelpieLease - Buggy Configuration (should fail LeaseUniqueness)
```bash
cd docs/tla
...
Expected output:
```
Error: Invariant LeaseUniqueness is violated.
State 1: /\ clock = 0 /\ leases = (a1 :> [holder |-> NONE, expiry |-> 0]) ...
State 2: /\ leases = (a1 :> [holder |-> n1, expiry |-> 3]) ...
State 3: /\ leases = (a1 :> [holder |-> n2, expiry |-> 3]) ...  (BOTH believe they hold it!)
```

### KelpieActorLifecycle - Safe Configuration (should pass)
```bash
cd docs/tla
